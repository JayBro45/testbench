...
- Reading electrical parameters required for AVR testing
...
class HiokiPW3336:
    ...
    def _mock_read(self, key: str) -> float:
        ...
        -------
        float
            Simulated numeric value.
        """
        base = self._mock_state[key]
        noise = random.uniform(-0.5, 0.5)

        if key in ("iin", "iout", "kwin", "kwout"):
            noise = random.uniform(-0.2, 0.2)

        value = round(max(0.0, base + noise), 3)
        self.logger.debug(f"MOCK READ | {key}={value}")
        return value
    ...
    def read_all(self) -> dict:
        """
        Perform a bulk read of enabled parameters.

        Returns
        -------
        dict
            Dictionary of parameter names mapped to numeric values.
            Only parameters enabled in config.json are included.
        """
        self.logger.debug("Bulk read started")
        data = {}

        if self._enabled_params.get("frequency"):
            data["frequency"] = self.read_frequency()

        if self._enabled_params.get("vin"):
            data["vin"] = self.read_voltage_in()

        if self._enabled_params.get("iin"):
            data["iin"] = self.read_current_in()

        if self._enabled_params.get("kwin"):
            data["kwin"] = self.read_power_in()

        if self._enabled_params.get("vout"):
            data["vout"] = self.read_voltage_out()

        if self._enabled_params.get("iout"):
            data["iout"] = self.read_current_out()

        if self._enabled_params.get("kwout"):
            data["kwout"] = self.read_power_out()

        if self._enabled_params.get("vthd_out"):
            data["vthd_out"] = self.read_vthd_out()

        if self._enabled_params.get("efficiency"):
